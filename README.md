...
- **Engineer Ernie** (`eng/`): engineering persona. `/eng:cr`, `/eng:cr-teammate`, `/eng:address-pr-feedback`, `/eng:pr-watcher`, `/eng:deploy`, `/eng:spike`, `/eng:coderabbit-config`, `/eng:shortcut`. `cr` is his master code-review skill (the single local review path the `~/dev` merge gate keys on); it routes to `cr-teammate` (review others' PRs), `address-pr-feedback` and `pr-watcher` (respond to feedback). Also spikes risky unknowns, configures CodeRabbit, and builds macOS Shortcuts. `deploy` is the PR-less deploy ceremony for the retry / recovery / rebuild cases `/land-and-deploy` cannot serve. Ships Ernie's PR-lifecycle enforcement hooks: the ship-PR gate, the merge-clearance gate, the deploy gate (no deploy outside a ceremony), the `/ship` sentinel, and the review stamp recorder (see `eng/README.md`).
