...
- **Its decisions live in `scripts/sensor-poll-lib.sh`, not in the sensor.** The sensor sources that lib (`BASH_SOURCE`-relative, so the executing copy binds its own dependency and both the repo checkout and the plugin cache work). Every function in it is pure: JSON strings in, JSON or a verdict string out, no side effects. That is where the settle conditions, the CodeRabbit status pick, the new-comment filter and the fingerprint live, so **a change to WHEN the sensor considers a round settled belongs in the lib, not in the polling script**. Its truth table is covered by `tests/sensor-poll.bats`, which sources the same lib directly; `tests/sensor-poll-integration.bats` drives the sensor end to end instead.
