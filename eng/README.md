...
| `/eng:deploy` | Deploy what is ALREADY on main, as a verified ceremony: the PR-less cases `/land-and-deploy` structurally cannot serve (retry after a failed deploy, recovery, a `--rebuild-base` rerun). Refuses when the branch has an open PR, shows the host-versus-main delta, runs the repo's declared `deploy.json` command, and gates the result on `devops check`. Arms the deploy gate, which is what makes it sanctioned rather than a hand-roll. |
...
| `deploy-gate.sh` | PreToolUse Bash + Skill + UserPromptSubmit | Sentinel and gate in one. Blocks a deploy-shaped command in an opted-in repo (`.deploy-gate.json`) unless `/land-and-deploy` or `/eng:deploy` armed the session. Two tiers: the entrypoint declared in `deploy.json`, and an ssh to a marker-listed host whose REMOTE command pulls, builds, or restarts. Read-only forms are never gated (`check` / `--status` / `--dry-run`, and ssh carrying only read verbs). Matching is per shell segment with a quote-aware splitter, so each invocation is judged on its own arguments and a quoted ssh payload stays intact. Arms its OWN kind (`deploy`), never the merge sentinel's `land`, so a deploy-only ceremony can never mint merge clearance. Break-glass: `DEPLOY_GATE_OVERRIDE="<reason>"` at command position. |
