...
swn_build_context() {
  ...
  case "$mode" in
    land)
      printf '%s' "Post-ship policy: /ship just opened PR #${num} (${url}). CodeRabbit is rate-limited (its notice contains 'rate limited by coderabbit.ai') and did not review this HEAD, but a current local /eng:cr review already backstops it (the same condition merge-clearance honors). There is nothing to watch: do NOT start /eng:pr-watcher. This PR is clear to land: run /land-and-deploy now. Do NOT ask the user how to land or present land-now / watch / pause options; landing is the standing default here. Only stop to ask if CI is red, there is a merge conflict, or the user explicitly said to pause."
      ;;
    review_then_land)
      printf '%s' "Post-ship policy: /ship just opened PR #${num} (${url}). CodeRabbit is rate-limited (its notice contains 'rate limited by coderabbit.ai') and will not review this HEAD, and no current local /eng:cr review backstops it. Do NOT open-endedly watch with /eng:pr-watcher. Run /eng:cr on this HEAD to backstop the rate-limited CodeRabbit, then land via /land-and-deploy. Do NOT ask the user how to land or present a menu of options; this backstop-then-land path is the standing default. Only stop to ask if /eng:cr surfaces a real blocker, CI is red, or the user explicitly said to pause."
      ;;
    *)
      printf '%s' "Post-ship policy: /ship just opened PR #${num} (${url}). Waiting for CodeRabbit is the standing default, so just do it: start the CodeRabbit watcher now by running /eng:pr-watcher ${url}, and once it settles clean, land via /land-and-deploy. Do NOT ask the user how to land or present land-now / watch-first / pause options; that decision is already made (always wait for CodeRabbit, then land). Only stop to ask if CI is red, there is a merge conflict, CodeRabbit raises a blocker you cannot auto-resolve, or the user explicitly said to pause. If the watcher reports CodeRabbit is rate-limited (a comment containing 'rate limited by coderabbit.ai', or it exits via cr_failure / already_settled with no real review), stop watching: instead ensure a current /eng:cr review backstops this HEAD, then land via /land-and-deploy."
      ;;
  esac
}
