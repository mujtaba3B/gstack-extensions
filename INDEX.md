...
| `skills/review-agent-pr/` | `/review-agent-pr <PR# \| URL>`. Review-only skill for PRs you did NOT author (use case: autonomous-agent PRs). Resolves the PR, runtime-checks the `pr-review-toolkit` plugin (offers to install if absent, else degraded inline review), fans the diff across six review lenses, verifies blocking findings against the real repo (not the PR's claims), gives a quick chat verdict, then posts ONE structured comment behind a confirm gate. Never merges/pushes/resolves/reassigns. No evals (v1). |
