...
Used by `pm:feature` (and future skills) to push back when a request is too big for a single issue, and to break it into a tracked epic with sequential child issues.
...
For each child in the planned order, re-enter `pm:feature` (do not try to file children directly from the gate — go through the normal feature flow so discovery, mockups, and preview all run). Pass the following `epic_context` blob into the next invocation:
...
The child invocation of `pm:feature` will:
