...
| `/qa:qa-plan` | Authors the two-phase QA plan (Development + Production) into the PR body before review, recommends a QA driver from `qa-roster.json`, presents the plan for human approval, and writes the approval stamp the QA-plan gates read. Step 3 of `~/dev/BUILD-PROCEDURE.md`. |
