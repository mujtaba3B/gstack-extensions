...
| `qa-plan-bash-build-gate.sh` | PostToolUse Bash | Gate 1b: the same rule for source written through Bash, which gate 1's matcher never saw. Decides from what the repo observed changing rather than from the command text, so a heredoc-fed interpreter, `sed -i`, `tee`, `python3 -c` and `eval` all land the same way. Being PostToolUse it INTERRUPTS rather than prevents: the write has already happened and is never reverted. See `docs/build-gate-coverage.md`. |
...
**The build gate has two halves, and only one of them can prevent a write.** `docs/build-gate-coverage.md` is the exact statement of what they do and do not intercept. Read it before claiming the gate covers something. The short version: gate 1 (`PreToolUse` Edit/Write) blocks before the write; gate 1b (`PostToolUse` Bash) fires after it, because deciding whether arbitrary shell writes source is undecidable from the command string, and the case that actually escaped on 2026-09-04 was a `python3 - <<'PY'` heredoc whose write lived inside the interpreter's source text. Gate 1b therefore observes the repo rather than parsing the command, which makes it blind to mechanism (heredocs, `-c`, `-e`, `eval`, `sed -i`, `tee`, `cp`, deletions) but leaves it unable to un-write anything. It never reverts your files. The PR gate is what keeps an unplanned branch from reaching a PR.
