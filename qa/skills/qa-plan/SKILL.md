...
4. **You cannot write the stamp without the human's click, and you should not try.** As of the approval-token fix (gstack-extensions#71) this is ENFORCED, not merely instructed. `write` refuses unless a `PostToolUse` hook has minted a token from a real `AskUserQuestion` answer of "Approve" on a question whose header matches `"QA plan"`, and it CONSUMES that token, so one click yields exactly one stamp.
   ...
   - **Tell the human about their two override routes, and do not try to use them yourself.** Neither is reachable by you, which is the point. (1) They send `qa-plan: I approve this plan` as a message on its own; a `UserPromptSubmit` hook mints the token from `.prompt`, a harness-filled field you cannot emit. Requires the hooks to be registered. (2) They run `qa-plan-stamp.sh override` in a real terminal tab; it demands a controlling terminal, which neither your Bash tool nor a `!`-prefixed command has, and it needs no hook at all, so it is the route that works when everything else is dormant. Say which one fits and stop. An override stamp records its own `approval_source` and expires after 8 hours, because it binds to no plan digest.
