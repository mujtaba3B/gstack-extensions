...
**The page must match what the digest hashes.** Step 6 hashes the PR-body `## QA` section and points the human at this page, so those two have to say the same thing about the plan. The Dev rows, Prod rows, production artifacts, and Definition of Done must carry the SAME content the digest is computed over; the only permitted differences are the three lines the template deliberately drops (the QA-driver line, `Standard (all green)`, and the QA-posture line), none of which are QA rows. If you edit the plan after publishing, refresh the artifact BEFORE recomputing the digest and re-asking, or the human approves a page that no longer matches the text their approval binds to.
