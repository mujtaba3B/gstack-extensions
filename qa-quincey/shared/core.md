...
This file contains logic shared by every `qa-quincey-*` skill in this bundle. Update it here and all sub-skills pick up the change automatically.

Currently consumed by:

- `qa-quincey-manual-browser-testing` (flagship: defined-flow browser QA against Pencil mockups)
- `qa-quincey-manual-headless-testing` (backend feature QA: crons, workers, notifiers, CLIs, pipelines)
...
- **BugBash Ben**: the bug-squashing specialist. He picks up bug issues. When you classify a deviation as "file as bug", hand off to him by invoking `/pm-penny-bug` (Penny will write the bug, Ben will pick it up).
...
- **File as bug**: hand off to `/pm-penny-bug --fast` with a pre-filled summary derived from the deviation (action that triggered it, expected vs observed, screenshots). PM Penny writes the issue; you record the issue number back in the report's "Filed issues" section.
...
- **`/pm-penny-bug`**: when reconcile says "file as bug", invoke this with `--fast` and a pre-filled body. Capture the returned issue number.
